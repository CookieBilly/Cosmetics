...
public class MorphChicken extends Morph
{
    ...
    public MorphChicken(final UUID uuid) {
    ...
    public void onClick() {
        this.activated = true;
        SoundEffect.ENTITY_CHICKEN_AMBIENT.playSound(this.getPlayer());
        int i = 0;
        final Item e;
        Bukkit.getScheduler().runTaskLater((Plugin)CookieGadgets.getInstance(), () -> {
            while (i <= 13) {
                this.getPlayer().getWorld().dropItem(this.getPlayer().getLocation(), ItemUtils.item(UUID.randomUUID().toString(), EnumMaterial.EGG));
