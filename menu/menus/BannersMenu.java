...
public class BannersMenu implements Listener
{
    public static void openBannersMenu(final PlayerManager playerManager, int n) {
        final int size = BannerType.enabled().size();
        ...
        final Inventory inventory = Bukkit.createInventory((InventoryHolder)null, 54, ChatUtil.format(String.valueOf(Category.BANNERS.getGUIName()) + StringUtils.addPlaceholders(EnumItem.PAGES.getDisplayName(), Arrays.asList("{CURRENT_PAGE}", "{MAX_PAGES}"), Arrays.asList(String.valueOf(n), String.valueOf(maxPagesAmount)))));
        int n2 = 0;
        int n3 = 1;
        if (n > 1) {
            n3 = 21 * (n - 1) + 1;
        ...
        for (int i = n3; i <= n4; ++i) {
            try {
                ...
                final BannerType bannerType = BannerType.enabled().get(i - 1);
                if (PermissionUtils.noPermission(player, bannerType.getPermission(), EnumPermission.BANNERS.getPermission(), false)) {
                    final DiscountManager discountManager = new DiscountManager(playerManager, Category.BANNERS.isPurchasable() && bannerType.isPurchasable(), bannerType.getMysteryDust());
                    if (EnumItem.NO_PERMISSION.showCustomItem()) {
                        InventoryUtils.inventory(inventory, bannerType.getDisplayName(), EnumItem.NO_PERMISSION.getCustomItem(), bannerType.getLockedLore(), EnumItem.NO_PERMISSION.isShowInLore() ? EnumItem.NO_PERMISSION.getLore() : null, discountManager.getLore(), GInventory.LAY_OUT_21.getLayOut()[n2++]);
                    }
                    else {
                        InventoryUtils.inventoryBanner(inventory, bannerType.getDisplayName(), bannerType.getGPatterns().getBaseColor(), bannerType.getGPatterns().getPatterns(), bannerType.getLockedLore(), EnumItem.NO_PERMISSION.isShowInLore() ? EnumItem.NO_PERMISSION.getLore() : null, discountManager.getLore(), GInventory.LAY_OUT_21.getLayOut()[n2++]);
                    }
                }
                else {
                    InventoryUtils.inventoryBannerAddGlow(inventory, bannerType.getDisplayName(), bannerType.getGPatterns().getBaseColor(), bannerType.getGPatterns().getPatterns(), bannerType.getUnlockedLore(), EnumItem.HAS_PERMISSION.isShowInLore() ? EnumItem.HAS_PERMISSION.getLore() : null, GInventory.LAY_OUT_21.getLayOut()[n2++], (playerManager.getEquippedBanner() == null) ? null : playerManager.getEquippedBanner().getDisplayName());
                }
            }
            catch (Exception ex) {
                ex.printStackTrace();
        ...
        if (EnumItem.RESET_BANNER.show()) {
            InventoryUtils.inventory(inventory, EnumItem.RESET_BANNER.getItemStack(), EnumItem.RESET_BANNER.getSlot());
        }
        if (n == 1) {
    ...
    @EventHandler
    public void onInvClickBanners(final InventoryClickEvent inventoryClickEvent) {
        final Player player = (Player)inventoryClickEvent.getWhoClicked();
        if (!inventoryClickEvent.getView().getTitle().startsWith(Category.BANNERS.getGUIName())) {
            return;
        }
        ...
        if (ItemUtils.getCurrentItem(inventoryClickEvent, EnumItem.RESET_BANNER.getItemStack(), EnumItem.RESET_BANNER.getSlot())) {
        ...
        if (ItemUtils.getCurrentItem(inventoryClickEvent, EnumItem.PREVIOUS_PAGE.getDisplayName(), EnumItem.PREVIOUS_PAGE.getSlot())) {
            openBannersMenu(playerManager, playerManager.getCurrentMenuPage() - 1);
            inventoryClickEvent.setCancelled(true);
            return;
        }
            ...
            if (inventoryClickEvent.getCurrentItem().getItemMeta().getDisplayName().equals(ChatUtil.format(bannerType.getDisplayName()))) {
                if (PermissionUtils.noPermission(player, bannerType.getPermission(), EnumPermission.BANNERS.getPermission(), false)) {
                    if (Category.BANNERS.isPurchasable()) {
                        ...
                        int n = bannerType.getMysteryDust();
                        if (itemCostDiscount != null) {
                            n = itemCostDiscount.getPriceAfterDiscount(bannerType.getMysteryDust());
                        }
                        if (playerManager.getMysteryDust() >= n) {
                            playerManager.purchaseData().setData(Category.BANNERS, bannerType.getName(), bannerType.getDisplayName(), itemCostDiscount, bannerType.getMysteryDust(), bannerType.getPermission());
                        ...
                        if (PermissionUtils.noPermission(player, bannerType.getPermission(), EnumPermission.BANNERS.getPermission(), true)) {
                    ...
                    try {
                        if (playerManager.getEquippedBanner() != null && ChatUtil.format(bannerType.getDisplayName()).equals(playerManager.getEquippedBanner().getDisplayName())) {
                            playerManager.unequipBanner();
                            player.sendMessage(MessageType.RESET_BANNER.getFormatMessage());
                            if (EnumItem.RESET_BANNER.isPlaySoundEnabled()) {
                                EnumItem.RESET_BANNER.getSound().playSound(player, 1.0f, 2.0f);
                        ...
                        if (!bannerType.checkRequirement(playerManager)) {
                            if (EnumItem.NO_PERMISSION.isPlaySoundEnabled()) {
                                EnumItem.NO_PERMISSION.getSound().playSound(player, 1.0f, 0.5f);
                            }
                            player.closeInventory();
                        ...
                        playerManager.equipBanner(bannerType);
                        player.sendMessage(MessageType.SELECT_BANNER.getFormatMessage().replace("{BANNER}", bannerType.getDisplayNameStripColor()));
                        ...
                        if (EnumItem.HAS_PERMISSION.isCloseGUIMenuAfterSelect()) {
                            ...
                            openBannersMenu(playerManager, playerManager.getCurrentMenuPage());
                        }
                        inventoryClickEvent.setCancelled(true);
                        return;
                    }
