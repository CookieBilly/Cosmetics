...
{
    public static void openHatsMenu(final PlayerManager playerManager, int n) {
        final int size = HatType.enabled().size();
        ...
        final Inventory inventory = Bukkit.createInventory((InventoryHolder)null, 54, ChatUtil.format(String.valueOf(Category.HATS.getGUIName()) + StringUtils.addPlaceholders(EnumItem.PAGES.getDisplayName(), Arrays.asList("{CURRENT_PAGE}", "{MAX_PAGES}"), Arrays.asList(String.valueOf(n), String.valueOf(maxPagesAmount)))));
        int n2 = 0;
        int n3 = 1;
        if (n > 1) {
            n3 = 27 * (n - 1) + 1;
        ...
        for (int i = n3; i <= n4; ++i) {
            try {
                ...
                final HatType hatType = HatType.enabled().get(i - 1);
                if (PermissionUtils.noPermission(player, hatType.getPermission(), EnumPermission.HATS.getPermission(), false)) {
                    final DiscountManager discountManager = new DiscountManager(playerManager, Category.HATS.isPurchasable() && hatType.isPurchasable(), hatType.getMysteryDust());
                    if (EnumItem.NO_PERMISSION.showCustomItem()) {
                        InventoryUtils.inventory(inventory, hatType.getDisplayName(), EnumItem.NO_PERMISSION.getCustomItem(), hatType.getLockedLore(), EnumItem.NO_PERMISSION.isShowInLore() ? EnumItem.NO_PERMISSION.getLore() : null, discountManager.getLore(), GInventory.LAY_OUT_27.getLayOut()[n2++]);
                    }
                    else {
                        InventoryUtils.inventory(inventory, hatType.getDisplayName(), hatType.getMaterial(), hatType.getLockedLore(), EnumItem.NO_PERMISSION.isShowInLore() ? EnumItem.NO_PERMISSION.getLore() : null, discountManager.getLore(), GInventory.LAY_OUT_27.getLayOut()[n2++]);
                    }
                }
                else {
                    InventoryUtils.inventoryAddGlow(inventory, hatType.getDisplayName(), hatType.getMaterial(), hatType.getUnlockedLore(), EnumItem.HAS_PERMISSION.isShowInLore() ? EnumItem.HAS_PERMISSION.getLore() : null, GInventory.LAY_OUT_27.getLayOut()[n2++], (playerManager.getEquippedHat() == null) ? null : playerManager.getEquippedHat().getDisplayName());
                }
            }
            catch (Exception ex) {
        ...
        if (EnumItem.RESET_HAT.show()) {
            InventoryUtils.inventory(inventory, EnumItem.RESET_HAT.getItemStack(), EnumItem.RESET_HAT.getSlot());
        }
        if (n == 1) {
    ...
    @EventHandler
    public void onInvClickHats(final InventoryClickEvent inventoryClickEvent) {
        final Player player = (Player)inventoryClickEvent.getWhoClicked();
        if (!inventoryClickEvent.getView().getTitle().startsWith(Category.HATS.getGUIName())) {
            return;
        }
        ...
        if (!Category.HATS.isEnabled() || !WorldUtils.isWorldEnabled(player, true)) {
        ...
        if (ItemUtils.getCurrentItem(inventoryClickEvent, EnumItem.RESET_HAT.getItemStack(), EnumItem.RESET_HAT.getSlot())) {
        ...
        if (ItemUtils.getCurrentItem(inventoryClickEvent, EnumItem.PREVIOUS_PAGE.getDisplayName(), EnumItem.PREVIOUS_PAGE.getSlot())) {
            openHatsMenu(playerManager, playerManager.getCurrentMenuPage() - 1);
            inventoryClickEvent.setCancelled(true);
            return;
        }
        ...
        for (final HatType hatType : HatType.values()) {
            if (inventoryClickEvent.getCurrentItem().getItemMeta().getDisplayName().equals(ChatUtil.format(hatType.getDisplayName()))) {
                if (PermissionUtils.noPermission(player, hatType.getPermission(), EnumPermission.HATS.getPermission(), false)) {
                    if (Category.HATS.isPurchasable()) {
                        ...
                        int n = hatType.getMysteryDust();
                        if (itemCostDiscount != null) {
                            n = itemCostDiscount.getPriceAfterDiscount(hatType.getMysteryDust());
                        }
                        if (playerManager.getMysteryDust() >= n) {
                            playerManager.purchaseData().setData(Category.HATS, hatType.getName(), hatType.getDisplayName(), itemCostDiscount, hatType.getMysteryDust(), hatType.getPermission());
                        ...
                        if (PermissionUtils.noPermission(player, hatType.getPermission(), EnumPermission.HATS.getPermission(), true)) {
                    ...
                    try {
                        if (playerManager.getEquippedHat() != null && ChatUtil.format(hatType.getDisplayName()).equals(playerManager.getEquippedHat().getDisplayName())) {
                            playerManager.unequipHat();
                            player.sendMessage(MessageType.RESET_HAT.getFormatMessage());
                            if (EnumItem.RESET_HAT.isPlaySoundEnabled()) {
                                EnumItem.RESET_HAT.getSound().playSound(player, 1.0f, 2.0f);
                        ...
                        if (!hatType.checkRequirement(playerManager)) {
                            if (EnumItem.NO_PERMISSION.isPlaySoundEnabled()) {
                                EnumItem.NO_PERMISSION.getSound().playSound(player, 1.0f, 0.5f);
                            }
                            player.closeInventory();
                        ...
                        playerManager.equipHat(hatType);
                        player.sendMessage(MessageType.SELECT_HAT.getFormatMessage().replace("{HAT}", hatType.getDisplayNameStripColor()));
                        ...
                        if (EnumItem.HAS_PERMISSION.isCloseGUIMenuAfterSelect()) {
                            ...
                            openHatsMenu(playerManager, playerManager.getCurrentMenuPage());
                        }
                        inventoryClickEvent.setCancelled(true);
                    }
                    catch (Exception ex) {
