...
public class CommandRemovePermission extends SubCommand
{
    public CommandRemovePermission() {
        super("/gmenu removeperm <cosmetic> <type> <player>", "Remove the permission to access cosmetic item from a player.", null, "CookieGadgets.commands.removepermission", new String[] { "removepermission", "removeperm", "rp" }, true);
    }
    
    @Override
    protected void onCommandPlayer(final Player player, final String[] array) {
        if (array.length != 4) {
            CommandManager.printMessage((CommandSender)player, new CommandRemovePermission());
            return;
        }
    ...
    protected void onOtherCommandSender(final CommandSender commandSender, final String[] array) {
        if (array.length != 4) {
            CommandManager.printMessage(commandSender, new CommandRemovePermission());
            return;
        }
    ...
    private void onCommand(final CommandSender commandSender, final Player player, final String[] array) {
        ...
        switch (s3 = s) {
            case "suitBoots": {
                ...
                if (playerManager.getEquippedSuitEquipment().get(EnumArmorType.BOOTS) == value2) {
                    playerManager.unequipSuitEquipment(EnumArmorType.BOOTS);
                }
                playerManager.removeUnlockedCosmeticItem(Category.SUITS, value2.getName());
                player.sendMessage(MessageType.NO_LONGER_HAVE_ACCESS_TO_SUIT.getFormatMessage().replace("{SUIT}", value2.getDisplayNameStripColor()));
                commandSender.sendMessage(MessageType.SUCCESSFUL_REMOVE_PERMISSION.getFormatMessage().replace("{TYPE}", value2.getDisplayNameStripColor()).replace("{PLAYER}", player.getName()));
                break;
            }
            case "suitHelmet": {
                if (!Category.SUITS.isEnabled()) {
                ...
                if (playerManager.getEquippedSuitEquipment().get(EnumArmorType.HELMET) == value4) {
                    playerManager.unequipSuitEquipment(EnumArmorType.HELMET);
                }
                playerManager.removeUnlockedCosmeticItem(Category.SUITS, value4.getName());
                player.sendMessage(MessageType.NO_LONGER_HAVE_ACCESS_TO_SUIT.getFormatMessage().replace("{SUIT}", value4.getDisplayNameStripColor()));
                commandSender.sendMessage(MessageType.SUCCESSFUL_REMOVE_PERMISSION.getFormatMessage().replace("{TYPE}", value4.getDisplayNameStripColor()).replace("{PLAYER}", player.getName()));
                break;
            }
            case "banner": {
                ...
                if (playerManager.getEquippedBanner() == value5) {
                    playerManager.unequipBanner();
                }
                playerManager.removeUnlockedCosmeticItem(Category.BANNERS, value5.getName());
                player.sendMessage(MessageType.NO_LONGER_HAVE_ACCESS_TO_BANNER.getFormatMessage().replace("{BANNER}", value5.getDisplayNameStripColor()));
                commandSender.sendMessage(MessageType.SUCCESSFUL_REMOVE_PERMISSION.getFormatMessage().replace("{TYPE}", value5.getDisplayNameStripColor()).replace("{PLAYER}", player.getName()));
                break;
            }
            case "gadget": {
                ...
                if (playerManager.getEquippedGadget() == value6) {
                    playerManager.unequipGadget();
                }
                playerManager.removeUnlockedCosmeticItem(Category.GADGETS, value6.getName());
                player.sendMessage(MessageType.NO_LONGER_HAVE_ACCESS_TO_GADGET.getFormatMessage().replace("{GADGET}", value6.getDisplayNameStripColor()));
                commandSender.sendMessage(MessageType.SUCCESSFUL_REMOVE_PERMISSION.getFormatMessage().replace("{TYPE}", value6.getDisplayNameStripColor()).replace("{PLAYER}", player.getName()));
                break;
            }
            case "suitChestplate": {
                if (!Category.SUITS.isEnabled()) {
                ...
                if (playerManager.getEquippedSuitEquipment().get(EnumArmorType.CHESTPLATE) == value8) {
                    playerManager.unequipSuitEquipment(EnumArmorType.CHESTPLATE);
                }
                playerManager.removeUnlockedCosmeticItem(Category.SUITS, value8.getName());
                player.sendMessage(MessageType.NO_LONGER_HAVE_ACCESS_TO_SUIT.getFormatMessage().replace("{SUIT}", value8.getDisplayNameStripColor()));
                commandSender.sendMessage(MessageType.SUCCESSFUL_REMOVE_PERMISSION.getFormatMessage().replace("{TYPE}", value8.getDisplayNameStripColor()).replace("{PLAYER}", player.getName()));
                break;
            }
            case "hat": {
                ...
                if (playerManager.getEquippedHat() == value9) {
                    playerManager.unequipHat();
                }
                playerManager.removeUnlockedCosmeticItem(Category.HATS, value9.getName());
                player.sendMessage(MessageType.NO_LONGER_HAVE_ACCESS_TO_HAT.getFormatMessage().replace("{HAT}", value9.getDisplayNameStripColor()));
                commandSender.sendMessage(MessageType.SUCCESSFUL_REMOVE_PERMISSION.getFormatMessage().replace("{TYPE}", value9.getDisplayNameStripColor()).replace("{PLAYER}", player.getName()));
                break;
            }
            case "pet": {
                ...
                if (playerManager.getEquippedPet() == value10) {
                    playerManager.unequipPet();
                }
                playerManager.removeUnlockedCosmeticItem(Category.PETS, value10.getName());
                player.sendMessage(MessageType.NO_LONGER_HAVE_ACCESS_TO_PET.getFormatMessage().replace("{PET}", value10.getDisplayNameStripColor()));
                commandSender.sendMessage(MessageType.SUCCESSFUL_REMOVE_PERMISSION.getFormatMessage().replace("{TYPE}", value10.getDisplayNameStripColor()).replace("{PLAYER}", player.getName()));
                break;
            }
            case "cloak": {
                ...
                if (playerManager.getEquippedCloak() == value11) {
                    playerManager.unequipCloak();
                }
                playerManager.removeUnlockedCosmeticItem(Category.CLOAKS, value11.getName());
                player.sendMessage(MessageType.NO_LONGER_HAVE_ACCESS_TO_CLOAK.getFormatMessage().replace("{CLOAK}", value11.getDisplayNameStripColor()));
                commandSender.sendMessage(MessageType.SUCCESSFUL_REMOVE_PERMISSION.getFormatMessage().replace("{TYPE}", value11.getDisplayNameStripColor()).replace("{PLAYER}", player.getName()));
                break;
            }
            case "emote": {
                ...
                if (playerManager.getEquippedEmote() == value12) {
                    playerManager.unequipEmote();
                }
                playerManager.removeUnlockedCosmeticItem(Category.EMOTES, value12.getName());
                player.sendMessage(MessageType.NO_LONGER_HAVE_ACCESS_TO_EMOTE.getFormatMessage().replace("{EMOTE}", value12.getDisplayNameStripColor()));
                commandSender.sendMessage(MessageType.SUCCESSFUL_REMOVE_PERMISSION.getFormatMessage().replace("{TYPE}", value12.getDisplayNameStripColor()).replace("{PLAYER}", player.getName()));
                break;
            }
            case "morph": {
                ...
                if (playerManager.getEquippedMorph() == value13) {
                    playerManager.unequipMorph();
                }
                playerManager.removeUnlockedCosmeticItem(Category.MORPHS, value13.getName());
                player.sendMessage(MessageType.NO_LONGER_HAVE_ACCESS_TO_MORPH.getFormatMessage().replace("{MORPH}", value13.getDisplayNameStripColor()));
                commandSender.sendMessage(MessageType.SUCCESSFUL_REMOVE_PERMISSION.getFormatMessage().replace("{TYPE}", value13.getDisplayNameStripColor()).replace("{PLAYER}", player.getName()));
                break;
            }
            case "miniature": {
                ...
                if (playerManager.getEquippedMiniature() == value14) {
                    playerManager.unequipMiniature();
                }
                playerManager.removeUnlockedCosmeticItem(Category.MINIATURES, value14.getName());
                player.sendMessage(MessageType.NO_LONGER_HAVE_ACCESS_TO_MINIATURE.getFormatMessage().replace("{MINIATURE}", value14.getDisplayNameStripColor()));
                commandSender.sendMessage(MessageType.SUCCESSFUL_REMOVE_PERMISSION.getFormatMessage().replace("{TYPE}", value14.getDisplayNameStripColor()).replace("{PLAYER}", player.getName()));
                break;
            }
            case "animated_hat": {
                ...
                if (playerManager.getEquippedAnimatedHat() == value15) {
                    playerManager.unequipAnimatedHat();
                }
                playerManager.removeUnlockedCosmeticItem(Category.ANIMATED_HATS, value15.getName());
                player.sendMessage(MessageType.NO_LONGER_HAVE_ACCESS_TO_ANIMATED_HAT.getFormatMessage().replace("{ANIMATED_HAT}", value15.getDisplayNameStripColor()));
                commandSender.sendMessage(MessageType.SUCCESSFUL_REMOVE_PERMISSION.getFormatMessage().replace("{TYPE}", value15.getDisplayNameStripColor()).replace("{PLAYER}", player.getName()));
                break;
            }
            case "particle": {
                ...
                if (playerManager.getEquippedParticle() == value16) {
                    playerManager.unequipParticle();
                }
                playerManager.removeUnlockedCosmeticItem(Category.PARTICLES, value16.getName());
                player.sendMessage(MessageType.NO_LONGER_HAVE_ACCESS_TO_PARTICLE.getFormatMessage().replace("{PARTICLE}", value16.getDisplayNameStripColor()));
                commandSender.sendMessage(MessageType.SUCCESSFUL_REMOVE_PERMISSION.getFormatMessage().replace("{TYPE}", value16.getDisplayNameStripColor()).replace("{PLAYER}", player.getName()));
                break;
            }
            case "suitLeggings": {
                if (!Category.SUITS.isEnabled()) {
                ...
                if (playerManager.getEquippedSuitEquipment().get(EnumArmorType.LEGGINGS) == value18) {
                    playerManager.unequipSuitEquipment(EnumArmorType.LEGGINGS);
                }
                playerManager.removeUnlockedCosmeticItem(Category.SUITS, value18.getName());
                player.sendMessage(MessageType.NO_LONGER_HAVE_ACCESS_TO_SUIT.getFormatMessage().replace("{SUIT}", value18.getDisplayNameStripColor()));
                commandSender.sendMessage(MessageType.SUCCESSFUL_REMOVE_PERMISSION.getFormatMessage().replace("{TYPE}", value18.getDisplayNameStripColor()).replace("{PLAYER}", player.getName()));
                break;
            }
            default:
                break;
