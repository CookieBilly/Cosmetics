...
public class CommandManager implements CommandExecutor
{
    ...
    public CommandManager(final Plugin plugin) {
        plugin.getServer().getPluginCommand("mysterydust").setExecutor((CommandExecutor)this);
        plugin.getServer().getPluginCommand("mysterydust").setAliases((List)Arrays.asList("dust"));
    }
    ...
    public static void printMessage(final CommandSender commandSender, final String str, final String s, String str2) {
    ...
    public static void printHelpMessage(final CommandSender commandSender, int n) {
        ...
        if (commandSender instanceof Player) {
            commandSender.sendMessage(ChatUtil.format("&e---------------&fHelp: Mystery Dust&e--------------- &f(" + n + "/" + maxPagesAmount + ")"));
            for (int n5 = n3; n5 <= n4 && n5 <= CommandManager.subCmds.size(); ++n5) {
                final SubCommand subCommand = CommandManager.subCmds.get(n5 - 1);
                CookieGadgets.getNMSManager().newJSONMessage(ChatUtil.format(" &e" + subCommand.getUsage() + " &b- " + subCommand.getDescription())).suggestCommand(subCommand.getUsage()).showText(ChatUtil.format("&b" + subCommand.getUsage() + "\n&7" + subCommand.getHoverText())).send((Player)commandSender);
            }
            if (n >= maxPagesAmount) {
                if (n > 1 && maxPagesAmount > 1) {
                    CookieGadgets.getNMSManager().newJSONMessage(ChatUtil.format("&3Use &b/mysterydust help " + (maxPagesAmount - 1) + " &3to display page " + (maxPagesAmount - 1) + "!")).runCommand("/mysterydust help " + (maxPagesAmount - 1)).send((Player)commandSender);
                }
            }
            else {
                CookieGadgets.getNMSManager().newJSONMessage(ChatUtil.format("&3Use &b/mysterydust help " + (n + 1) + " &3to display page " + (n + 1) + "!")).runCommand("/mysterydust help " + (n + 1)).send((Player)commandSender);
            }
        }
        else {
            commandSender.sendMessage(ChatUtil.format("&e---------------&fHelp: Mystery Dust&e--------------- &f(" + n + "/" + maxPagesAmount2 + ")"));
            ...
            if (n >= maxPagesAmount2) {
                if (n > 1 && maxPagesAmount2 > 1) {
                    commandSender.sendMessage(ChatUtil.format("&3Use &b/mysterydust help " + (maxPagesAmount2 - 1) + " &3to display page " + (maxPagesAmount2 - 1) + "!"));
                }
            }
            else {
                commandSender.sendMessage(ChatUtil.format("&3Use &b/mysterydust help " + (n + 1) + " &3to display page " + (n + 1) + "!"));
            }
        }
    }
