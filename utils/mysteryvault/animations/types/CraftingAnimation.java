...
public class CraftingAnimation extends Animation
{
    ...
    static {
        CraftingAnimation.pitch = new float[] { 0.5f, 0.529732f, 0.561231f, 0.594604f, 0.629961f, 0.66742f, 0.707107f, 0.749154f, 0.793701f, 0.840896f, 0.890899f, 0.943874f, 1.0f, 1.059463f, 1.122462f, 1.189207f, 1.259921f, 1.33484f, 1.414214f, 1.498307f, 1.587401f, 1.681793f, 1.781797f, 1.887749f, 2.0f };
    }
    
    public CraftingAnimation(final PlayerManager playerManager, final MysteryVault mysteryVault, final Quality quality, final long n) {
        super(playerManager, mysteryVault, AnimationType.CRAFTING, quality, n);
        this.volume = 0.05f;
    ...
    private void spawnMysteryBoxQualityBlock(final Player player, final int n, final Location location) {
        Bukkit.getScheduler().runTaskLater((Plugin)CookieGadgets.getInstance(), (Runnable)new Runnable() {
            ...
            public void run() {
                location.clone();
                final Location location = new Location(location.getWorld(), location.getX() + 0.5, location.getY() - 0.25, location.getZ() + 0.5, GMiniBlock.getYawByBlockFace(CraftingAnimation.this.blockFace), 0.0f);
                if (CraftingAnimation.this.blockFace == BlockFace.EAST) {
                    location.setX(location.getX() + 0.75);
                }
                else if (CraftingAnimation.this.blockFace == BlockFace.WEST) {
                    location.setX(location.getX() + 0.25);
                }
                else if (CraftingAnimation.this.blockFace == BlockFace.SOUTH) {
                    location.setZ(location.getZ() + 0.75);
                }
                else if (CraftingAnimation.this.blockFace == BlockFace.NORTH) {
                ...
                e.setGravity(false);
                e.setSmall(true);
                e.setHeadPose(CraftingAnimation.this.getAnimation().getQualityBlock().getEulerAngle());
                try {
                    e.setMarker(false);
                ...
                e.setHelmet(CraftingAnimation.this.getAnimation().getQualityBlock().getItemStack());
                ...
                CraftingAnimation.this.starArmorStand.add(e);
                new BukkitRunnable() {
                    ...
                    public void run() {
                        ...
                        if (this.step <= 7) {
                            ...
                            if (CraftingAnimation.this.blockFace == BlockFace.EAST || CraftingAnimation.this.blockFace == BlockFace.WEST) {
                                add.setX(this.oldLoc.getX());
                            }
                            else if (CraftingAnimation.this.blockFace == BlockFace.SOUTH || CraftingAnimation.this.blockFace == BlockFace.NORTH) {
    ...
    private void playSound(final SoundEffect soundEffect, final int n, final long n2) {
        new BukkitRunnable() {
            public void run() {
                soundEffect.playSound(CraftingAnimation.this.getMysteryVault().getLocation().clone(), 1.0f, CraftingAnimation.pitch[n]);
            }
